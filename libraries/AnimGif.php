...
class AnimGif
{
	...
	*/
	 ...
	 */
	public function create($frames, $durations = self::DEFAULT_DURATION, $loop = 0, $num_colors = null, $dither = true, $transparent = -1)
	{
		...
		foreach ($frames as $frame) {
			if (is_resource($frame)) { // in-memory image resource (hopefully)
				...
				// reduce the number of colors if wanted
				if (isset($num_colors)) {
					imagetruecolortopalette($frame, $dither, $num_colors);  //  compress reduce number of colors in palette
				}
			...
			if ($i == 0) {
				if (isset($transparent)) {
					$this->transparent_color = hexdec($transparent);
				} else {
